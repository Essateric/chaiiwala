// sendFeedbackEmail.js (Fully Converted to ESM)
...
async function sendRealEmail(pdfBuffer, fileName, recipientEmail = 'essateric@gmail.com') {
  ...
  const mailOptions = {
    from: emailUser,
    to: recipientEmail,
   subject: `Customer Feedback Form Submitted - ${pdfFileName}`,
    text: `PDF "${fileName}" attached.\n\nGenerated by Essateric Solutions.`,
    html: `<h2>Customer Feedback Form Submitted</h2><p>PDF "<strong>${fileName}</strong>" is attached.</p>`,
    attachments: [
      {
        filename: fileName,
        content: pdfBuffer,
...
export async function handler(event) {
  ...
  try {
    ...
    if (payload.location || payload.customer_name) {
      ...
      const recipient = 'usman.aftab@chaiiwala.co.uk';
