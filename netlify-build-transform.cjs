...
// Convert any js references to cjs when we're in ESM mode
if (packageData.type === 'module') {
  console.log('🔄 Converting js imports to cjs where needed in command');
  command = command
    .replace(/build-netlify-vite-fallback\.js/g, 'build-netlify-vite-fallback.cjs')
    .replace(/build-netlify\.js/g, 'build-netlify.cjs');
}
