...
export default function StoreStockUpdatePage() {
  ...
  const isStoreManager = user?.role === 'store' || user?.permissions === 'store';
  ...
  const filteredStockItems = combinedStockData.filter(item => {
    ...
    const threshold = Number(item.lowStockThreshold ?? 1) || 0;
    const isOutOfStock = quantity === 0;
    // Keep existing logic: "Low" excludes true out-of-stock
  ...
  // Helper to determine stock status for styling (badge)
  ...
  // Helper to get stock status text (badge)
                  ...
                  <TableBody>
                    {filteredStockItems.length === 0 ? (
                      ...
                      filteredStockItems.map((item) => {
                        ...
                        // ✅ Highlight rule (store managers only): include zero as low when threshold > 0
                        const limit = Number(item.lowStockThreshold ?? 1) || 0;
                        const isLow = limit > 0 && Number(editedQuantity) <= limit; // includes 0
                        const rowClass = [
                          hasChanged ? "bg-blue-50" : "",
                          isStoreManager && isLow ? "bg-red-50 hover:bg-red-50/80 border-l-4 border-red-400" : ""
                        ].join(" ").trim();

                        return (
                          <TableRow key={item.id} className={rowClass} aria-label={isLow ? "Low stock row" : undefined}>
