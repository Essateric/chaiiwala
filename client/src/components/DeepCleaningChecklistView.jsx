// src/components/DeepCleaningChecklistView.jsx
 ...
 *  - readOnly?: boolean (optional; when true forces read-only)
 *  - storeIdOverride?: number|string (optional; when provided, component shows that store)
 *
 * If you render for regional/admin from a parent page, you can do:
 * <DeepCleaningChecklistView profile={profile} readOnly storeIdOverride={selectedStoreId} />
 */
export default function DeepCleaningChecklistView({
  profile,
  readOnly = false,
  ...
  const visibleStores = useMemo(() => {
    if (isAdminOrRegional) return stores;
    return stores.filter((s) => (profile?.store_ids || []).includes(s.id));
  }, [stores, profile, isAdminOrRegional]);

  // Resolve active store id
  const resolvedStoreId = useMemo(() => {
    if (storeIdOverride != null && storeIdOverride !== "") {
      return Number(storeIdOverride);
    }
    if (isStoreManager) {
      return (profile?.store_ids && profile.store_ids[0]) || null;
    }
    if (selectedStore !== "auto") {
      return Number(selectedStore);
    }
    return null; // regional/admin must pick a store (or pass storeIdOverride)
  }, [storeIdOverride, isStoreManager, profile, selectedStore]);

  // Load deep_cleaning rows for the current week (+ store if known)
  const loadWeek = async () => {
    setIsLoading(true);
    let query = supabase
      .from("deep_cleaning")
      .select(
        "id, task, start, end_time, completed_at, store_id, store_name, anytime"
      )
      .gte("start", weekStart.toISOString())
      .lte("start", weekEnd.toISOString());

    // If we know the store, let Supabase filter for perf
    if (resolvedStoreId) {
  ...
  const handleToggle = async (task, date) => {
    if (!resolvedStoreId) {
      toast({
        ...
        description: "Select a store to tick cells.",
        variant: "destructive",
      });
      return;
    }
    if (isReadOnly) {
      // Read-only view: no ticking
      return;
    }
    ...
    if (existing) {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id === existing.id ? { ...r, completed_at: newCompletedAt } : r
        )
      );
      return;
    }

    // Insert new completed cell (store managers only)
    ...
    const storeName =
      (stores.find((s) => s.id === resolvedStoreId) || {}).name || "";
  ...
  const doneCells = useMemo(() => {
    ...
    for (let i = 0; i < taskTypes.length; i++) {
      for (let j = 0; j < days.length; j++)
        if (tickState(taskTypes[i], days[j])) count++;
    }
    return count;
      ...
      <div className="rounded-lg border bg-muted/30 p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col gap-2 items-stretch md:flex-row md:items-center">
            {/* Regional/Admin choose a store unless parent passes storeIdOverride */}
            {isAdminOrRegional && storeIdOverride == null ? (
        ...
        <table className="w-full border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-gray-100">
              <th className="p-2 text-left text-sm font-semibold text-gray-700 sticky left-0 bg-gray-100 z-10">
                Deep Cleaning Tasks
              </th>
              {days.map((d) => (
                <th
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="p-4 text-sm text-gray-600">
                  Select a store to view the weekly matrix.
                </td>
              </tr>
            ) : (
              taskTypes.map((task) => (
                <tr key={task} className="border-t">
                  <td className="p-2 text-sm text-gray-800 sticky left-0 bg-white z-10 max-w-[320px]">
                    {task}
                  </td>
                  {days.map((d) => {
                    const done = tickState(task, d);
                    const isToday = isSameDay(d, new Date());
                    const canToggle = !!resolvedStoreId && !isReadOnly;
                    return (
                      <td
                        key={task + format(d, "yyyy-MM-dd")}
                      ...
                      >
                        <button
                          ...
                          disabled={!canToggle}
                          aria-label={`Toggle ${task} for ${format(
                            d,
                            "EEE d MMM"
                          )}`}
                          title={
